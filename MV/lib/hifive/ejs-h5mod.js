...
(function() {
	var h = function(a, b) {
			for(var c = b.exec(a), d = [], e; null != c;) e = c.index, 0 != e && (a.substring(0, e), d.push(a.substring(0, e)), a = a.slice(e)), d.push(c[0]), a = a.slice(c[0].length), c = b.exec(a);
			"" == !a && d.push(a);
			return d
		},
		i = function(a, b) {
	...
	EJS = function(a) {
		...
		if(a.precompiled) this.template = {}, this.template.process = a.precompiled, EJS.update(this.name, this);
			...
			if(a.element) {
				...
				var b = a.url,
					c = EJS.get(this.name, this.cache);
				if(c) return c;
				if(c == EJS.INVALID_PATH) return null;
				try {
	...
	EJS.Scanner.to_text = function(a) {
		return null ==
			a || void 0 === a ? "" : a instanceof Date ? a.toDateString() : a.toString ? a.toString() : ""
	};
	EJS.Scanner.prototype = {
		scan: function(a) {
			var b = this.SplitRegexp;
			if("" == !this.source)
				for(var c = h(this.source, /\n/), d = 0; d < c.length; d++) this.scanline(c[d], b, a)
		},
	...
	EJS.Compiler.prototype = {
		compile: function(a, b) {
			...
			this.scanner.scan(function(a, b) {
				if(null == b.stag) switch(a) {
					case "\n":
						d +=
							"\n";
						c.push('___ViewO.push("' + e(d) + '");');
						c.cr();
						d = "";
						break;
					case b.left_delimiter:
					case b.left_equal:
					case b.left_equal_noescape:
					case b.left_comment:
						b.stag = a;
						0 < d.length && c.push('___ViewO.push("' + e(d) + '")');
						d = "";
						break;
					case b.double_left:
						d += b.left_delimiter;
					...
					case b.right_delimiter:
						switch(b.stag) {
							case b.left_delimiter:
								"\n" == d[d.length - 1] ? (d = d.substr(0, d.length - 1), c.push(d), c.cr()) : c.push(d);
								break;
							case b.left_equal_noescape:
								...
								break;
							case b.left_equal:
								c.push("___ViewO.push((EJS.Scanner.to_text(h5.u.str.escapeHtml(" + d + "))))")
						}
						b.stag = null;
						d = "";
						break;
			...
			c.close();
			this.out = c.script + ";";
			var g = "/*" + b + "*/this.process = function(_CONTEXT,_VIEW) { try { with(_VIEW) { with (_CONTEXT) {" + this.out + " return ___ViewO.join('');}}}catch(e){e.lineNumber=null;throw e;}};";
			try {
				eval(g)
			} catch(h) {
				if("undefined" != typeof JSLINT) {
					JSLINT(this.out);
					for(var f = 0; f < JSLINT.errors.length; f++)
						if(g = JSLINT.errors[f], "Unnecessary semicolon." != g.reason) throw g.line++, f = Error(), f.lineNumber = g.line, f.message = g.reason, a.view && (f.fileName = a.view), f;
				} else throw h;
			}
		}
	};
	EJS.config = function(a) {
		...
		EJS.get = function(a, d) {
			return !1 == d ? null : b[a] ? b[a] : null
		};
		EJS.update = function(a, d) {
			null != a && (b[a] = d)
		};
		EJS.INVALID_PATH = -1
	};
